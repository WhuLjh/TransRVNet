...
class ResizedCrop_:
    def __init__(self, size: Union[int, Tuple[int], List[int]],
                 scale: [float, float, float, float, float, float] = [0.5, 0.75, 1, 1.25, 1.5, 1.75],
                 seg_fill: int = 0) -> None:
        """Resize the input image to the given size.
        """
        self.size = size
    ...
    def __call__(self, sample: list) -> list:
        ...
        nH = int(sample['img'].shape[1] * scale)
        nW = int(sample['img'].shape[2] * scale)
...
def get_train_augmentation(size: Union[int, Tuple[int], List[int]], seg_fill: int = 0):
    return Compose([
        ...
        ResizedCrop_(size, scale=[0.5, 0.75, 1, 1.25, 1.5], seg_fill=seg_fill),
        Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225))
    ])

def get_train_augmentation_adapt(size: Union[int, Tuple[int], List[int]], seg_fill: int = 0):
    return Compose([
        # RandomColorJitter(p=0.2),
        # RandomHorizontalFlip(p=0.5),
        # RandomGaussianBlur((3, 3), p=0.2),

...
from semseg.models.utils import SkePoint_prompted_BufSAM,SkeLine_connected_SLIC
...
class CMNeXt(BaseModel):
    ...
    def generate_pl_pre(self, images, predictor,img,lbl,epoch,ep,name,savedir):
        pre = self.forward(images)
        if epoch < ep:
            label = lbl.cpu().detach().numpy()
            if predictor != None:
                corrected_pl = SkePoint_prompted_BufSAM(img, label.copy(), predictor, name,epoch,(3,3),savedir,False)
            else:
                corrected_pl = SkeLine_connected_SLIC(img, label.copy(),name,epoch,2000,savedir,False)
            corrected_pl = torch.tensor(corrected_pl).to('cuda:0').long()
            return pre,corrected_pl
        else:
            return pre, lbl.long()

...
def evaluate(model, dataloader, device):
    ...
    for data_ in tqdm(dataloader):
        images = [x.to(device) for x in data_[0]]
        labels = data_[1].to(device)
...
@torch.no_grad()
def train_pre(model, dataloader, device, epoch):
    n_classes = dataloader.dataset.n_classes
    print('Evaluating train...')
    model.eval()
    sliding = False
    for images, labels, img, name in tqdm(dataloader):
        images = [x.to(device) for x in images]
        labels = labels.to(device)
        if sliding:
            pred = sliding_predict(model, images, num_classes=n_classes).softmax(dim=1)
        else:
            pred = model(images).softmax(dim=1)

        pred = pred.argmax(dim=1)
        for i in range(pred.shape[0]):
            pre_ = pred[i, :, :].cpu().detach().numpy().astype(np.uint8)

            savepath = 'output_adapt/Mydata_adapt_CMNeXt-B2_ibon/process/pre/' + str(
                epoch) + '/'
            if not os.path.exists(savepath):
                os.makedirs(savepath)
            cv2.imwrite(savepath + name[i], pre_)
            pre_[pre_ == 1] = 255
            savepath = 'output_adapt/Mydata_adapt_CMNeXt-B2_ibon/process/pre_color/' + str(
                epoch) + '/'
            if not os.path.exists(savepath):
                os.makedirs(savepath)
            cv2.imwrite(savepath + name[i], pre_)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default=r'E:\code\DELIVER-main\configs\flair_gr_rgbhn.yaml')
